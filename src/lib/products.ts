...
export async function getCatalogProducts(): Promise<CatalogProduct[]> {
  try {
    ...
    const { data, error } = await db.from("catalog_products").select("id,title,description,category,image_url,product_url,gallery_images,preco_cliente_base,preco_revendedor_atacado,is_active").eq("is_active", true).order("category").order("title");
