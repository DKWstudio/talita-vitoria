...
export default function ResellerDocuments() {
  ...
  async function upload(type: string, event: ChangeEvent<HTMLInputElement>) {
    ...
    setSending(type); setMessage(""); const { data: auth } = await supabase.auth.getUser(); const user = auth.user;
    if (!user) { setMessage("Faça login novamente para enviar documentos."); setSending(""); return; }
    const extension = file.name.split('.').pop() || 'file'; const path = `${user.id}/${type}.${extension}`;
    ...
    setMessage(error ? error.message : "Documento enviado com segurança para análise."); setSending("");
  }
