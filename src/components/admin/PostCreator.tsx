...
const dimensions: Record<Format, [number, number]> = {
  post: [1080, 1350],
  story: [1080, 1920],
  whatsapp: [1080, 1920],
};
...
export default function PostCreator({ products }: { products: ManagedProduct[] }) {
  ...
  const caption = product ? `✨ ${product.title}\n\n${product.description || "Enxovais e detalhes escolhidos com carinho para sua casa."}\n\n${audience === "cliente" ? `Valor para cliente: ${money}.` : `Condições especiais para revenda: ${money}.`}\n\n📲 Peça pelo WhatsApp: (49) 98856-8055\n🔗 ${productUrl}\n\n#TalitaVitoria #BordadosVitoria #Enxovais #CamaMesaEBanho #Chapeco` : "";
  ...
  function createImage() {
    ...
    image.onload = () => {
      ...
      context.clip();
      drawCover(context, image, width, imageHeight);
      context.restore();
      context.fillStyle = "#34445f";
      context.fillRect(0, imageHeight - 30, width, height - imageHeight + 30);
      context.fillStyle = "#f1c1c8";
      context.font = "bold 31px Arial";
      context.fillText("TALITA VITÓRIA", 72, imageHeight + 76);
      ...
      words.forEach((word) => {
        ...
        if (context.measureText(next).width > width - 144 && line) {
