...
async function getAdminData() {
  ...
  const [productsResult, rulesResult, clicksResult, ordersResult] = await Promise.all([
    supabase
      .from("products")
    ...
    supabase
      ...
      .order("created_at", { ascending: false })
      .limit(2000),
    supabase.from("orders").select("id, customer_name, whatsapp, city, delivery_type, profile, status, total, created_at, order_items(product_title, quantity, unit_price)").order("created_at", { ascending: false }).limit(100),
  ]);
...
function OrdersTable({ orders }: { orders: Array<Record<string, unknown>> }) {
  if (orders.length === 0) return <p className="rounded-xl border border-dashed border-slate-300 bg-white p-6 text-sm text-slate-500">Nenhuma pré-venda registrada ainda.</p>;
  return <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white"><table className="min-w-[900px] w-full text-left text-sm"><thead className="bg-slate-100 text-[11px] font-black uppercase tracking-widest text-slate-500"><tr><th className="p-3">Pedido</th><th className="p-3">Cliente</th><th className="p-3">Itens</th><th className="p-3">Entrega</th><th className="p-3">Total</th><th className="p-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100">{orders.map((order) => { const items = Array.isArray(order.order_items) ? order.order_items as Array<Record<string, unknown>> : []; return <tr key={String(order.id)} className="align-top"><td className="p-3"><p className="font-bold">#{String(order.id).slice(0,8)}</p><p className="text-xs text-slate-500">{new Date(String(order.created_at)).toLocaleString("pt-BR")}</p></td><td className="p-3"><p className="font-bold">{String(order.customer_name)}</p><a className="text-xs text-emerald-700" href={`https://wa.me/${String(order.whatsapp).replace(/\D/g, "")}`} target="_blank">WhatsApp</a></td><td className="p-3 text-xs">{items.map((item) => <p key={String(item.product_title)}>{String(item.quantity)}× {String(item.product_title)}</p>)}</td><td className="p-3 text-xs"><p>{String(order.city)}</p><p className={order.delivery_type === "propria" ? "text-emerald-700" : "text-amber-700"}>{order.delivery_type === "propria" ? "Entrega própria" : "Sob consulta"}</p></td><td className="p-3 font-black">{formatCurrency(Number(order.total))}</td><td className="p-3"><form action={updateOrderStatus} className="flex gap-2"><input type="hidden" name="id" value={String(order.id)} /><select name="status" defaultValue={String(order.status)} className="rounded-lg border border-slate-200 px-2 py-1 text-xs"><option value="novo">Novo</option><option value="em_contato">Em contato</option><option value="confirmado">Confirmado</option><option value="entregue">Entregue</option><option value="cancelado">Cancelado</option></select><button className="rounded-lg bg-slate-950 px-2 py-1 text-xs font-bold text-white">Salvar</button></form></td></tr>})}</tbody></table></div>;
}
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-8 text-slate-950">
      <section className="mx-auto max-w-7xl space-y-8">
        ...
        <section className="space-y-4"><div><h2 className="text-xl font-black uppercase">Pré-vendas</h2><p className="text-sm text-slate-500">Acompanhe pedidos e atualize o status de atendimento.</p></div><OrdersTable orders={data.orders} /></section>
